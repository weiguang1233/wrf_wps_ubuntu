#!/usr/bin/env bash
#
# Version-selectable WRF + WPS installer for native Ubuntu x86_64 (Intel or AMD).
#
# Source archives may be supplied in BASE_DIR:
#   v<WRF_VERSION>.tar.gz
#   WPS-<WPS_VERSION>.tar.gz
...
usage() {
  cat <<'EOF'
Usage:
  bash install_wrf_wps.sh [options]

Options:
  --wrf-version V  Pinned WRF version. Default: 4.8.0; 4.8 aliases 4.8.0.
                   WPS is selected from the pinned version-pair catalog.
  --list-versions  List supported pinned WRF/WPS pairs and exit.
  --base DIR       Existing installation/source-archive directory.
                   Default: installations/wrf-V-wps-V under this script's
...
If dependencies and source archives are already available:
  bash install_wrf_wps.sh --skip-apt --offline
...
while (($#)); do
  case "$1" in
    --wrf-version)
      (($# >= 2)) || { echo "ERROR: --wrf-version requires a version" >&2; exit 2; }
      wrf_version="$2"
      shift 2
      ;;
    --list-versions)
...
[[ -r "$versions_file" ]] \
  || { echo "ERROR: version catalog not found: $versions_file" >&2; exit 2; }
if ((list_versions)); then
  awk '!/^#/ && NF == 4 {printf "WRF %s -> WPS %s\n", $1, $2}' "$versions_file"
  exit 0
fi
wrf_version="${wrf_version#[vV]}"
[[ "$wrf_version" != "4.8" ]] || wrf_version="4.8.0"
mapfile -t selected_versions < <(awk -v version="$wrf_version" \
  '!/^#/ && NF == 4 && $1 == version {print}' "$versions_file")
((${#selected_versions[@]} == 1)) || {
  echo "ERROR: unsupported or duplicate WRF version: $wrf_version; use --list-versions" >&2
  exit 2
}
read -r wrf_version wps_version wrf_git_commit wps_git_commit <<<"${selected_versions[0]}"
[[ "$wrf_version" =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?$ \
   && "$wps_version" =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?$ \
   && "$wrf_git_commit" =~ ^[0-9a-f]{40}$ \
   && "$wps_git_commit" =~ ^[0-9a-f]{40}$ ]] || {
  echo "ERROR: invalid version catalog entry" >&2
  exit 2
}
if [[ -z "$base_dir" ]]; then
  base_dir="$script_dir/installations/wrf-$wrf_version-wps-$wps_version"
  [[ ! "$base_dir" =~ [[:space:]] ]] || {
...
wrf_archive_name="v$wrf_version.tar.gz"
wrf_archive="$base_dir/$wrf_archive_name"
wps_archive_name="WPS-$wps_version.tar.gz"
wps_archive="$base_dir/$wps_archive_name"
wrf_archive_url="https://github.com/wrf-model/WRF/releases/download/v$wrf_version/$wrf_archive_name"
wps_archive_url="https://github.com/wrf-model/WPS/archive/refs/tags/v$wps_version.tar.gz"
...
temp_dir="$(mktemp -d /tmp/wrf-wps-install.XXXXXX)"
download_part_files=()
extract_stage_dirs=()
...
echo "============================================================"
echo "WRF $wrf_version / WPS $wps_version one-click installer"
...
expected_wrf_sha="$(
  awk -v name="$wrf_archive_name" '$2 == name {print $1}' "$checksums_file"
)"
expected_wps_sha="$(
  awk -v name="$wps_archive_name" '$2 == name {print $1}' "$checksums_file"
)"
...
if [[ -L "$wrf_dir" ]]; then
  ...
  grep -Fxq "WRF Model Version $wrf_version" "$wrf_dir/README" \
    || fail "$wrf_dir is not WRF $wrf_version"
fi
...
if [[ -L "$wps_dir" ]]; then
  ...
  grep -Fxq "WRF Pre-Processing System Version $wps_version" "$wps_dir/README" \
    || fail "$wps_dir is not WPS $wps_version"
fi
...
extract_archive_source() {
  ...
  grep -Fxq "$expected_readme_text" "$stage/README" \
...
if ((wrf_preexisting)); then
  wrf_source_actual="$(
    describe_preexisting_source "$wrf_source_marker" WRF "$wrf_version"
  )"
...
if ((wps_preexisting)); then
  wps_source_actual="$(
    describe_preexisting_source "$wps_source_marker" WPS "$wps_version"
  )"
...
if ((wrf_preexisting == 0)); then
  if [[ "$source_method" == "archive" ]]; then
    ...
    extract_archive_source \
      "$wrf_archive" "$wrf_dir" "WRF Model Version $wrf_version"
    write_source_marker \
      "$wrf_source_marker" WRF archive "$wrf_version" \
      "$wrf_archive_url" "$wrf_git_commit" "$expected_wrf_sha"
    wrf_source_actual="archive-verified"
  else
    clone_git_source \
      "$wrf_git_url" "v$wrf_version" "$wrf_git_commit" \
      "$wrf_dir" "WRF Model Version $wrf_version" 1
    write_source_marker \
      "$wrf_source_marker" WRF git "$wrf_version" \
...
if ((wps_preexisting == 0)); then
  if [[ "$source_method" == "archive" ]]; then
    ...
    extract_archive_source \
      "$wps_archive" "$wps_dir" "WRF Pre-Processing System Version $wps_version"
    write_source_marker \
      "$wps_source_marker" WPS archive "$wps_version" \
      "$wps_archive_url" "$wps_git_commit" "$expected_wps_sha"
    wps_source_actual="archive-verified"
  else
    clone_git_source \
      "$wps_git_url" "v$wps_version" "$wps_git_commit" \
      "$wps_dir" "WRF Pre-Processing System Version $wps_version" 0
    write_source_marker \
      "$wps_source_marker" WPS git "$wps_version" \
...
grep -Fxq "WRF Pre-Processing System Version $wps_version" "$wps_dir/README" \
  || fail "$wps_dir is not WPS $wps_version"
echo "SOURCE_TREES_OK: WRF $wrf_version + WPS $wps_version"
...
fatal_build_regex='undefined reference|collect2: error|ld returned [0-9]+ exit status|cannot find -l|No rule to make target|compilation terminated|fatal error:|(^|[[:space:]])error:'
...
if wrf_install_ready; then
  ...
  (
    cd "$wrf_dir"
    printf '%s\n' -1 | env -u JASPERLIB -u JASPERINC \
      NETCDF="$netcdf_prefix" NETCDF_C="$netcdf_prefix" \
      WRFIO_NCD_LARGE_FILE_SUPPORT=1 NETCDF_classic=1 ./configure
  ) >"$wrf_dir/configure-menu.log" 2>&1 || true
  wrf_config_option="$(awk '/GNU \(gfortran\/gcc\)/ {
    for (i=1; i<NF; i++) if ($(i+1) == "(dmpar)") {
      n=$i; sub(/\.$/, "", n); print n
    }
  }' "$wrf_dir/configure-menu.log")"
  [[ "$wrf_config_option" =~ ^[1-9][0-9]*$ ]] \
    || fail "could not uniquely identify GNU dmpar; see $wrf_dir/configure-menu.log"
  echo "Configuring WRF: GNU dmpar ($wrf_config_option), basic nesting (1)..."
  (
    cd "$wrf_dir"
    printf '%s\n1\n' "$wrf_config_option" \
  ...
  # GCC 15 defaults to C23; older WRF sources use pre-C23 declarations.
...
if ((wps_ready)); then
  ...
  (
    cd "$wps_dir"
    printf '%s\n' -1 | env NETCDF="$netcdf_prefix" WRF_DIR="$wrf_dir" \
      ./configure --build-grib2-libs
  ) >"$wps_dir/configure-menu.log" 2>&1 || true
  wps_config_option="$(awk '/^[[:space:]]*[0-9]+\..*Linux.*x86_64.*gfortran.*\(serial\)/ {
    n=$1; sub(/\.$/, "", n); print n
  }' "$wps_dir/configure-menu.log")"
  [[ "$wps_config_option" =~ ^[1-9][0-9]*$ ]] \
    || fail "could not uniquely identify GNU serial; see $wps_dir/configure-menu.log"
  echo "Configuring WPS: GNU serial ($wps_config_option), internal GRIB2 libraries..."
  (
    cd "$wps_dir"
    printf '%s\n' "$wps_config_option" \
